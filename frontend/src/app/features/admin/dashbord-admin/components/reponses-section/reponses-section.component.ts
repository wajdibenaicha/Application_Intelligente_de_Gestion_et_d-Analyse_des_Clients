...
@Component({
  selector: 'app-admin-reponses-section',
  standalone: true,
  encapsulation: ViewEncapsulation.None,
  imports: [CommonModule, FormsModule],
  templateUrl: './reponses-section.component.html'
})
export class AdminReponsesSectionComponent implements OnChanges {
  @Input() questionnaires: any[] = [];
  @Input() adminReponses: any[] = [];
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if ((changes['chartSent'] || changes['chartRepondu']) && this.adminReponses.length > 0) {
      setTimeout(() => this.renderChart(), 150);
    }
    if (changes['adminReponses'] && (!this.adminReponses || this.adminReponses.length === 0)) {
  ...
  private renderChart() {
    ...
    this.chart = new Chart(canvas, {
      ...
      data: {
        ...
        datasets: [{ data: [this.chartSent, this.chartRepondu],
